...
bool Collator::isSymbol(wchar_t character)
{
...
ASFUNCTIONBODY_ATOM(Collator,compare)
{
	...
	try
    {
	    ...
	    tiny_string string1;
	    tiny_string string2;
	    ARG_UNPACK_ATOM(string1);
	    ARG_UNPACK_ATOM(string2);
		std::string s1 = string1.raw_buf();
		std::string s2 = string2.raw_buf();
	    std::locale::global(l);
		int32_t value = th->compareStrings(s1, s2);
...
ASFUNCTIONBODY_ATOM(Collator,equals)
{
	...
	try
    {
		...
		std::string s2 = string2.raw_buf();
	    std::locale::global(l);
		//LOG(LOG_NOT_IMPLEMENTED,std::to_string(th->compareStrings(s1, s2)));
